...
%     rectPre=arrRect(:,1); 
    ...
    %%%%%%%%%%%%%% Transformation matrix for previous image %%%%%%%%%%%%%%%
	%%% step-4.1 : %%%
    meanA = sum(matchedPoints1.Location)/size(matchedPoints1.Location,1);
	meanA_matrix = [1 0 -meanA(1,1); 0 1 -meanA(1,2); 0 0 1];
	avgPointsA = matchedPoints1.Location - repmat(meanA,size(matchedPoints1.Location,1),1);
	relDistA = sqrt(sum(avgPointsA.^2,2));
	sa = sqrt(2)/mean(relDistA);
	scaleA = [sa 0 0; 0 sa 0; 0 0 1];
    %Transformation matrix Ta
	Ta = scaleA * meanA_matrix;	
    
    %Normalized points 
    points_a_3xn = [matchedPoints1.Location ones(size(matchedPoints1.Location,1),1)]';
	norm_points_a_3xn = Ta * points_a_3xn;
	matchedPoints1_norm = norm_points_a_3xn'; 	
    
    %%%%%%%%%%%%%% Transformation matrix for current image %%%%%%%%%%%%%%%%
    %%% step-4.2 : %%%
    meanB = sum(matchedPoints2.Location)/size(matchedPoints2.Location,1);
	meanB_matrix = [1 0 -meanB(1,1); 0 1 -meanB(1,2); 0 0 1];
	avgPointsB = matchedPoints2.Location - repmat(meanB,size(matchedPoints2.Location,1),1);
	relDistB = sqrt(sum(avgPointsB.^2,2));
	sb = sqrt(2)/mean(relDistB);
	scaleB = [sb 0 0; 0 sa 0; 0 0 1];
    %Transformation matrix Tb
	Tb = scaleB * meanB_matrix;	
    
    %Normalized points 
    points_b_3xn = [matchedPoints2.Location ones(size(matchedPoints2.Location,1),1)]';
	norm_points_b_3xn = Tb * points_b_3xn;
	matchedPoints2_norm = norm_points_b_3xn'; 
    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%% Formulate A_matrix %%%%%%%%%%%%%%%%%%%%%%%%%
    %%% step-4.3 : %%%
    A_matrix = zeros(size(matchedPoints1_norm,1),9);
	for ii=1:size(matchedPoints1_norm,1)
	    ua = matchedPoints1_norm(ii,1);
	    va = matchedPoints1_norm(ii,2);
	    ub = matchedPoints2_norm(ii,1);
	    vb = matchedPoints2_norm(ii,2);
	    A_matrix(ii,:) = [ua*ub va*ub ub ua*vb va*vb vb ua va 1];
    end
    
    %%%%%%%%%%%%%%%%%%% Evaluate the equation using SVD %%%%%%%%%%%%%%%%%%%
    %%% step-4.4 : %%%
	[~, ~, V] = svd(A_matrix);
	f = V(:,end);
	T = reshape(f,[3 3])';
...
disp('overall error using Least Square Method (without RANSAC) ...');
disp(eucl_error/50);
if bOutputVideo
close(vHandle);
end
